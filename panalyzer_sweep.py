...
def panalyzertop(filename, mode='cumulative_simple', masking=0):
    ...
    dirname = filename + str(masking)
    logpath = top_dir / "Panalyzer_results" / filename
    # Setup AVF computation results Paths
    result_path = top_dir / "Panalyzer_results" / dirname
    ...
    if fin_csv.exists():
        ...
        # f_length = 10000000
        with open(logpath / (filename + '_log.txt'), 'r') as forigin:
    ...
    if mode == "cumulative_simple":  # This Branch computes the cumulative A/PVFs through the program runtime
        ...
        ace_parallel = parallel_ace(fin_csv, reg_num, n_datapoint, flength).multiprocessing_ace(thread, startp,
                                                                                                ...
                                                                                                '32bitsimple', masking)
    elif mode == "cumulative_detailed":  # This Branch computes the cumulative A/PVFs through the program runtime
        # Cumulative Analyzer configurations
        reg_num = 16
        considered_reg_num = 15
        n_datapoint = 6000
...
def panalyzertop64(filename, mode='cumulative'):
    start_time = time.perf_counter()  # Time counter starts
    ...
    if fin_csv.exists():
        ...
        # Convert trace files from raw text to csv
        f_length, lineoffset = txt2csv(filename, trace_dir=top_dir / 'gem5traces' / 'exec_traces', arch='arm64')
        flength = f_length  # By default, we are analyzing the entire trace file

    if mode == "cumulative":  # This Branch computes the cumulative A/PVFs through the program runtime
        # Cumulative Analyzer configurations
        reg_num = 32
        considered_reg_num = 32
        n_datapoint = 6000
...
if __name__ == '__main__':
    ...
    # ap.add_argument("-masking", "--average_masking", required=False, help="Average Masking Factor")
    # args_traceut = vars(ap.parse_args())['filename']
    # args_mode = vars(ap.parse_args())['mode']
    # args_masking = vars(ap.parse_args())['average_masking']
    # panalyzertop(args_traceut, args_mode, args_masking)
    ...
    for tracefile in f:
        for i in steps:
            panalyzertop(tracefile, 'cumulative_simple', masking=i)
            print('sweep ', i, ' done')
        print(tracefile + ' Done')
