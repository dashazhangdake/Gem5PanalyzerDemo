...
class ace_calculators:
    ...
    def ace_calculator_detailed(self, start, length, sample_ratio, lineoffset, mode='64bit', m=0):
        reg_n = self.reg_num
        if mode == '32bitsimple':
            info = csv2np(self.fin, start, length, self.reg_num).arm32simple(lineoffset, m)
        elif mode == '32bitdetailed':
            info = csv2np(self.fin, start, length, self.reg_num).arm32detailed(lineoffset)
        elif mode == '64bit':
            info = csv2np(self.fin, start, length, self.reg_num).arm64simple()
        else:
            info = 0
            print('type in correct mode')
            pass
    ...
    def ace_calculator_quanta(self, start, length, lineoffset, mode='64bit'):
        reg_n = self.reg_num
        if mode == '32bit':
            info = csv2np(self.fin, start, length, self.reg_num).arm32detailed(lineoffset)
        else:
