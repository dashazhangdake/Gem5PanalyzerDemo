...
def wr_extractor32(reg_id, dst1, dst2, src1, src2, op, data, val_prev):
...
def wr_extractor64(reg_id, dst1, dst2, src1, src2, op, data, val_prev):
    # print('w' + reg_id)
    # InfoExtractor already assigned operands on the correct field based on the type of operation
    if 'w' + reg_id == src1 or 'w' + reg_id == src2 or 'x' + reg_id == src1 or 'x' + reg_id == src2:
        r_entry = 1
    else:
        r_entry = 0

    if 'w' + reg_id == dst1 or 'w' + reg_id == dst2 or 'x' + reg_id == dst1 or 'x' + reg_id == dst2:
        # If R[k] at line[i] is the dst operand: w=1 and reg_val=D
        w_entry = 1
        reg_val = data
    else:
        w_entry = 0  # Otherwise, w[k][i]=0, reg_val[k][i] = reg_val[k][i-1]
