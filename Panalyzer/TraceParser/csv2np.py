...
from Panalyzer.utils.wr_extractor import wr_extractor32
from Panalyzer.utils.wr_extractor import wr_extractor64
...
class csv2np:
    ...
    def arm32detailed(self, lineoffset=0):
        ...
        with open(self.trace_csv, mode='r') as info_csv:
            ...
            for i, row in enumerate(info_reader):  # Tick[i], or ith row in file
                ...
                for k in range(self.num_reg):  # kth register
                    ...
                    wr_list[k, 0, i] = \
                        wr_extractor32(reg_name, op_dst1, op_dst2, op_src1, op_src2, op_id, data, val_prev)[0]
                    wr_list[k, 1, i] = \
                        wr_extractor32(reg_name, op_dst1, op_dst2, op_src1, op_src2, op_id, data, val_prev)[1]
                    reg_val_table[k, i] = \
                        wr_extractor32(reg_name, op_dst1, op_dst2, op_src1, op_src2, op_id, data, val_prev)[2]
            # if len(src1_list) <= 0:
    ...
    def arm64simple(self, lineoffset=0):
        detailded_info = {'wr': None, 'regval': None, 'tick': None, 'masking': None, 'src1': None, 'src2': None,
                          'op': None}
        # Tick and opname
        tick_list = np.zeros([self.num_lines], dtype=np.int64)
        ...
        with open(self.trace_csv, mode='r') as info_csv:
            ...
            for i, row in enumerate(info_reader):  # Tick[i], or ith row in file
                ...
                for k in range(self.num_reg):  # kth register
                    val_prev = reg_val_table[k, i - 1]
                    reg_name = str(k)  # fp, lr, sp ,pc are renamed, simply
                    wr_list[k, 0, i] = \
                        wr_extractor64(reg_name, op_dst1, op_dst2, op_src1, op_src2, op_id, data, val_prev)[0]
                    wr_list[k, 1, i] = \
                        wr_extractor64(reg_name, op_dst1, op_dst2, op_src1, op_src2, op_id, data, val_prev)[1]
                    reg_val_table[k, i] = \
                        wr_extractor64(reg_name, op_dst1, op_dst2, op_src1, op_src2, op_id, data, val_prev)[2]
            # if len(src1_list) <= 0:
            #     src1_list.append('dummy')
            # if len(src2_list) <= 0:
            #     src2_list.append('dummy')
            ...
            m_table = arm32masking_calculator(self.num_lines, op_list, src1_list, src2_list, self.num_reg,
                                              reg_val_table).lmasking_calculator64()
            detailded_info['wr'] = wr_list
            detailded_info['regval'] = reg_val_table
            detailded_info['tick'] = tick_list
            detailded_info['src1'] = src1_list
    ...
    def arm32simple(self, lineoffset=0, m=0.12):
        detailded_info = {'wr': None, 'regval': None, 'tick': None, 'masking': None, 'src1': None, 'src2': None,
                          'op': None}
        # Tick and opname
        tick_list = np.zeros([self.num_lines], dtype=np.int64)
        ...
        with open(self.trace_csv, mode='r') as info_csv:
            ...
            m_table = arm32masking_calculator(self.num_lines, op_list, src1_list, src2_list, self.num_reg,
                                              reg_val_table).lmasking_calculator32simple(m)
            detailded_info['wr'] = wr_list
            detailded_info['regval'] = reg_val_table
            detailded_info['tick'] = tick_list
            detailded_info['src1'] = src1_list
...
if __name__ == "__main__":
    ...
    fname = "prime.csv"
    T = csv2np(csv_dir / fname, 0, 10, 32).arm64simple()
